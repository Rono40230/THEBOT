...
class MACD:
    ...
    def __init__(self, config: Optional[MACDConfig] = None):
        ...
        logger.debug(f"MACD initialisé: {self.config.fast_period}/{self.config.slow_period}/{self.config.signal_period}")
    
    def calculate(self, data: pd.DataFrame, 
                 include_signals: bool = True) -> Dict[str, any]:
        """
    ...
    def plot(self, data: pd.DataFrame, 
        ...
        try:
            ...
            figure = self.plotter.create_subplot_figure(
                result['data'], 
                signals, 
                title
            )
