...
class CandlePatternsCalculator:
    ...
    def detect_hammer(self, metrics: Dict[str, Decimal]) -> bool:
        ...
        return (metrics['lower_wick_ratio'] >= self.config.wick_ratio_threshold * metrics['body_ratio'] and
                metrics['upper_wick_ratio'] <= metrics['body_ratio'] and
                metrics['body_ratio'] >= self.config.min_body_ratio)
    
    def detect_shooting_star(self, metrics: Dict[str, Decimal]) -> bool:
        """Detect Shooting Star pattern"""
        return (metrics['upper_wick_ratio'] >= self.config.wick_ratio_threshold * metrics['body_ratio'] and
                metrics['lower_wick_ratio'] <= metrics['body_ratio'] and
                metrics['body_ratio'] >= self.config.min_body_ratio)
    ...
    def calculate_from_data(self, data: MarketData) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            'strength': strength,
            'metrics': metrics,
            'is_bullish_pattern': pattern in [PatternType.HAMMER, PatternType.BULLISH_ENGULFING],
            'is_bearish_pattern': pattern in [PatternType.SHOOTING_STAR, PatternType.BEARISH_ENGULFING],
