...
class SqueezeConfig:
    ...
    def __init__(
        ...
        assert (
            Decimal("0.5") <= bollinger_std <= Decimal("5.0")
        ), "bollinger_std must be between 0.5 and 5.0"
        assert 5 <= keltner_period <= 200, "keltner_period must be between 5 and 200"
        assert (
            Decimal("0.5") <= keltner_atr_multiplier <= Decimal("5.0")
        ), "keltner_atr_multiplier must be between 0.5 and 5.0"
    ...
    def get_required_periods(self) -> int:
        ...
        return (
            max(self.bollinger_period, self.keltner_period, self.momentum_length) + 5
