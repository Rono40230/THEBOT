...
class BreakoutCalculator:
    ...
    def add_data(self, data: MarketData) -> None:
        ...
        if len(self.volume_history) >= 10:
            self.avg_volume = Decimal(
                str(statistics.mean(list(self.volume_history)[-10:]))
            )
    ...
    def detect_breakout(self, current_data: MarketData) -> Dict[str, Any]:
        ...
        if current_price > resistance * (
            Decimal("1.0") + self.config.breakout_threshold
        ):
        ...
        if (
            self.avg_volume
            and current_volume > self.avg_volume * self.config.volume_multiplier
        ):
        ...
        if resistance_breakout:
            ...
            breakout_strength = strength_ratio * (
                Decimal("2.0") if volume_confirmed else Decimal("1.0")
            )
