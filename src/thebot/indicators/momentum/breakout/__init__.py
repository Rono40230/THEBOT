...
class BreakoutIndicator(BaseIndicator):
    ...
    def generate_signal(self, current_result: IndicatorResult) -> Optional[Signal]:
        ...
        breakout_strength = Decimal(current_result.metadata.get('breakout_strength', '0.0'))
        volume_confirmed = current_result.metadata.get('volume_confirmed', False)
        
        if not breakout_detected or breakout_strength < self.breakout_config.strength_threshold:
            return None
        ...
        if volume_confirmed and breakout_strength > self.breakout_config.strength_threshold * 2:
