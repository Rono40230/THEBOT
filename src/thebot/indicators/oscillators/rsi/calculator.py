...
class RSICalculator:
    ...
    def _calculate_smoothed_rsi(
        self, gain: Decimal, loss: Decimal
    ) -> Optional[Decimal]:
        ...
        if self.config.smoothing_method == "ema":
            ...
            self._avg_gain = (
                wilder_alpha * gain + (Decimal("1") - wilder_alpha) * self._avg_gain
            )
            self._avg_loss = (
                wilder_alpha * loss + (Decimal("1") - wilder_alpha) * self._avg_loss
            )
    ...
    def get_volatility_adjusted_levels(self) -> dict:
        ...
        rsi_variance = sum((rsi - rsi_mean) ** 2 for rsi in recent_rsi) / len(
            recent_rsi
        )
        ...
        volatility_factor = min(
            Decimal("1.5"), max(Decimal("0.5"), rsi_std / Decimal("10"))
        )

        adjusted_overbought = self.config.overbought_level + (
            Decimal("10") * volatility_factor
        )
        adjusted_oversold = self.config.oversold_level - (
            Decimal("10") * volatility_factor
        )
