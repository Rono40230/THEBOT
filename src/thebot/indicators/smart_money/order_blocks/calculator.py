...
from .config import (
    OrderBlockConfig, OrderBlockType, OrderBlockStatus, 
    OrderBlockStrength, get_style_config
)
...
class OrderBlock:
    ...
    def age_bars(self) -> int:
        ...
        if self.right_time and self.left_time:
            return int((self.right_time - self.left_time).total_seconds() / 3600)  # Approximation
        return 0
...
class OrderBlockCalculator:
    ...
    def analyze_blocks(self, data: pd.DataFrame) -> List[OrderBlock]:
        ...
        for i in range(self.config.lookback_period, len(df) - self.config.min_impulse_bars):
    ...
    def _prepare_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['wick_ratio'] = df['total_wick'] / (df['body_abs'] + 0.0001)  # Éviter division par 0
    ...
    def _detect_bullish_block(self, df: pd.DataFrame, idx: int) -> Optional[OrderBlock]:
        ...
        volume_ratio = current_bar['volume'] / df['volume_ma'].iloc[idx] if df['volume_ma'].iloc[idx] > 0 else 1.0
    ...
    def _calculate_strength_score(self, body_size: float, wick_ratio: float, 
                                 volume_ratio: float, impulse_strength: float) -> float:
    ...
    def _cleanup_blocks(self):
        ...
        self.blocks = [b for b in self.blocks if b.status != OrderBlockStatus.EXPIRED or b.age_bars < 100]
        ...
        if len(self.blocks) > self.config.max_blocks_display:
            ...
            self.blocks.sort(key=lambda x: (x.strength_score, -x.formation_bar), reverse=True)
        ...
        for block in self.blocks:
            ...
            for existing in merged:
                if (existing.type == block.type and 
                    self._blocks_overlap(existing, block)):
            ...
            if overlapping:
                ...
                overlapping.strength_score = max(overlapping.strength_score, block.strength_score)
    ...
    def _blocks_overlap(self, block1: OrderBlock, block2: OrderBlock) -> bool:
        ...
        overlap_size = max(0, min(block1.top, block2.top) - max(block1.bottom, block2.bottom))
    ...
    def get_strong_blocks(self, min_strength: OrderBlockStrength = OrderBlockStrength.STRONG) -> List[OrderBlock]:
    ...
    def get_trading_signals(self, current_price: float) -> Dict[str, Any]:
        ...
        if closest_block.type == OrderBlockType.BULLISH:
            if current_price <= closest_block.top and current_price >= closest_block.bottom:
                signals['signal'] = 'buy'
                signals['strength'] = closest_block.strength_score
                signals['recommendation'] = f"Achat sur Order Block bullish (force: {closest_block.strength_level.value})"
        
        elif closest_block.type == OrderBlockType.BEARISH:
            if current_price <= closest_block.top and current_price >= closest_block.bottom:
                signals['signal'] = 'sell'
                signals['strength'] = closest_block.strength_score
                signals['recommendation'] = f"Vente sur Order Block bearish (force: {closest_block.strength_level.value})"
        
        # Ajuster le niveau de risque
        if closest_block.strength_level in [OrderBlockStrength.STRONG, OrderBlockStrength.VERY_STRONG]:
    ...
    def export_blocks_data(self) -> List[Dict[str, Any]]:
        ...
        for block in self.blocks:
            export_data.append({
                ...
                'top': block.top,
                'bottom': block.bottom,
                'left_time': block.left_time.isoformat() if block.left_time else None,
...
def analyze_market_structure(data: pd.DataFrame) -> Dict[str, Any]:
    ...
    for i in range(high_period, len(data) - high_period):
        # Swing High
        if all(data['high'].iloc[i] >= data['high'].iloc[i-j] for j in range(1, high_period+1)) and \
           all(data['high'].iloc[i] >= data['high'].iloc[i+j] for j in range(1, high_period+1)):
            swing_highs.append({
                'index': i,
                'time': data.index[i],
        ...
        # Swing Low
        if all(data['low'].iloc[i] <= data['low'].iloc[i-j] for j in range(1, low_period+1)) and \
           all(data['low'].iloc[i] <= data['low'].iloc[i+j] for j in range(1, low_period+1)):
            swing_lows.append({
                'index': i,
                'time': data.index[i],
...
def find_order_block_signals(blocks: List[OrderBlock], current_price: float) -> Dict[str, Any]:
    """
    ...
    if nearest_block and min_distance < 0.01:  # Moins de 1% de distance
        if nearest_block.type == OrderBlockType.BULLISH and current_price <= nearest_block.top:
            signal = 'buy'
            strength = nearest_block.strength_score * (1 - min_distance * 10)  # Plus proche = plus fort
        elif nearest_block.type == OrderBlockType.BEARISH and current_price >= nearest_block.bottom:
            signal = 'sell'
