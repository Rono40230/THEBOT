...
class OrderBlockPlotter:
    ...
    def add_blocks_to_chart(
        self, fig: go.Figure, blocks: List[OrderBlock], data: pd.DataFrame
    ) -> go.Figure:
        """
    ...
    def _add_single_block(
        self, fig: go.Figure, block: OrderBlock, data: pd.DataFrame
    ) -> go.Figure:
    ...
    def _add_block_label(
        self, fig: go.Figure, block: OrderBlock, start_time: datetime
    ) -> go.Figure:
    ...
    def _add_legend_info(self, fig: go.Figure, blocks: List[OrderBlock]) -> go.Figure:
        ...
        strong_count = len(
            [
                ...
                for b in blocks
                if b.strength_level
                in [OrderBlockStrength.STRONG, OrderBlockStrength.VERY_STRONG]
            ]
        )
    ...
    def create_statistics_summary(self, blocks: List[OrderBlock]) -> Dict[str, Any]:
        ...
        # Bloc le plus fort
        strongest_block = (
            max(blocks, key=lambda x: x.strength_score) if blocks else None
        )
...
def create_order_blocks_overlay(
    data: pd.DataFrame, config: OrderBlockConfig
) -> Dict[str, Any]:
    """
    ...
    overlay_data = {
        ...
        "total_blocks": len(blocks),
        "active_blocks": len(
            [b for b in blocks if b.status == OrderBlockStatus.ACTIVE]
        ),
        "strong_blocks": len(
            [
                b
        ...
        "trading_signals": (
            calculator.get_trading_signals(data["close"].iloc[-1])
            if len(data) > 0
            else {}
        ),
    }
