...
from .plotter import OrderBlockPlotter, create_order_blocks_overlay
...
def analyze_order_blocks(
    data, style: str = "day_trading", create_chart: bool = False, **kwargs
):
    """
...
def quick_test():
    ...
    dates = pd.date_range(
        start=datetime.now() - timedelta(days=30), periods=720, freq="H"
    )
    np.random.seed(42)
