        ...
        if self.type == FVGType.BULLISH:
            ...
            if current_price <= self.top:
                ...
                if current_price <= self.bottom:
                    ...
                    self.fill_percentage = ((self.top - current_price) / self.size) * 100
            ...
            if current_price >= self.bottom:
                ...
                if current_price >= self.top:
                    ...
                    # Gap partiellement comblé
                    self.fill_percentage = ((current_price - self.bottom) / self.size) * 100
                    self.status = FVGStatus.PARTIALLY_FILLED
...
class FVGCalculator:
    ...
    def calculate_gaps(self, data: pd.DataFrame) -> List[FairValueGap]:
        ...
        try:
            ...
            for idx, row in data.iterrows():
                candle_data = {
                    'timestamp': idx if hasattr(idx, 'to_pydatetime') else datetime.now(timezone.utc),
                    'open': float(row['open']),
                    'high': float(row['high']),
                    'low': float(row['low']),
    ...
    def update_gaps_status(self, gaps: List[FairValueGap], data: pd.DataFrame) -> List[FairValueGap]:
        """
        ...
        current_time = data.index[-1] if hasattr(data.index[-1], 'to_pydatetime') else datetime.now(timezone.utc)
    ...
    def _create_bullish_gap(self, candle1: Dict, candle2: Dict, candle3: Dict) -> Optional[FairValueGap]:
    ...
    def _create_bearish_gap(self, candle1: Dict, candle2: Dict, candle3: Dict) -> Optional[FairValueGap]:
    ...
    def _validate_gap(self, gap: FairValueGap, creation_candle: Dict) -> bool:
        ...
        upper_wick = creation_candle['high'] - max(creation_candle['open'], creation_candle['close'])
        lower_wick = min(creation_candle['open'], creation_candle['close']) - creation_candle['low']
        if body_size > 0:
    ...
    def get_gaps_near_price(self, price: float, tolerance: float = 0.5) -> List[FairValueGap]:
    ...
    def calculate_gap_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'avg_gap_size': np.mean([gap.size for gap in self.gaps]) if self.gaps else 0,
            'avg_fill_time': self._calculate_avg_fill_time(),
            'strongest_gaps': sorted(active_gaps, key=lambda g: g.strength, reverse=True)[:5]
        }
    
    def _calculate_avg_fill_time(self) -> Optional[float]:
        """Calcule le temps moyen de remplissage des gaps en heures."""
        filled_gaps = [gap for gap in self.gaps if gap.fill_time and gap.first_touch_time]
        if not filled_gaps:
            return None
