...
from .calculator import FairValueGap, FVGType, FVGStatus
...
class FVGPlotter:
    ...
    def _add_gap_label(self, 
        ...
        text_color = self.config.bullish_gap_color if gap.type == FVGType.BULLISH else self.config.bearish_gap_color
    ...
    def create_gaps_summary_table(self, gaps: List[FairValueGap]) -> go.Figure:
        ...
        headers = [
            "ID", "Type", "Status", "Size (%)", "Fill (%)", 
            "Strength", "Age", "Touches", "Volume Conf."
        ]
    ...
    def create_gaps_statistics_chart(self, statistics: Dict[str, Any]) -> go.Figure:
        ...
        fig = make_subplots(
            rows=2, cols=2,
            subplot_titles=[
                "Gap Distribution", "Gap Status", 
                "Gap Sizes", "Fill Rate Trend"
            ],
            specs=[
                [{"type": "pie"}, {"type": "bar"}],
        ...
        fig.update_layout(
            title="📈 Fair Value Gaps Analytics",
            height=600,
            showlegend=True
        )
    ...
    def get_gaps_for_alerts(self, 
                           gaps: List[FairValueGap], 
                           current_price: float,
                           proximity_threshold: float = 0.5) -> List[Dict[str, Any]]:
        """
    ...
    def _generate_alert_message(self, gap: FairValueGap, current_price: float) -> str:
        ...
        return (f"Price {direction} {gap_type} FVG at {gap.mid_point:.4f} "
