...
class FVGAnalyzer:
    ...
    def analyze_gaps(self, 
                    data: pd.DataFrame,
                    update_existing: bool = True) -> List[FairValueGap]:
        """
    ...
    def get_strong_gaps(self, min_strength: float = 0.7) -> List[FairValueGap]:
        ...
        return [gap for gap in self._gaps_cache 
                if gap.strength >= min_strength and gap.is_active()]
    
    def get_gaps_near_price(self, 
                           current_price: float,
                           percentage_range: float = 2.0) -> List[FairValueGap]:
        """
        ...
        for gap in self._gaps_cache:
            if gap.is_active():
                if (gap.bottom <= current_price + price_range and 
                    gap.top >= current_price - price_range):
    ...
    def create_visualization(self, 
        ...
        fig = self.plotter.add_gaps_to_figure(
            fig, self._gaps_cache, x_data
        )
    ...
    def get_trading_signals(self, current_price: float) -> Dict[str, Any]:
        ...
        if bullish_gaps and not bearish_gaps:
            ...
            signals['strength'] = sum(g.strength for g in bullish_gaps) / len(bullish_gaps)
            signals['recommendations'].append("Prix près de gaps bullish - opportunité d'achat")
            
        elif bearish_gaps and not bullish_gaps:
            signals['signal'] = 'BEARISH' 
            signals['strength'] = sum(g.strength for g in bearish_gaps) / len(bearish_gaps)
            signals['recommendations'].append("Prix près de gaps bearish - prudence ou vente")
    ...
    def _merge_gaps(self, 
                   existing_gaps: List[FairValueGap], 
                   new_gaps: List[FairValueGap]) -> List[FairValueGap]:
...
def create_fvg_analyzer(trading_style: Union[str, TradingStyle] = 'day_trading',
                       custom_params: Optional[Dict[str, Any]] = None) -> FVGAnalyzer:
    """
...
def create_fvg_config(gap_threshold: float = 0.1,
