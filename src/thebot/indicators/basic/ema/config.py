    ...
    def validate(self) -> None:
            ...
            if not isinstance(self.smoothing_factor, Decimal):
                try:
                    ...
                    raise ConfigurationError(
                        "smoothing_factor must be convertible to Decimal"
                    )

            if self.smoothing_factor <= 0 or self.smoothing_factor >= 1:
                raise ConfigurationError(
                    "smoothing_factor must be between 0 and 1 (exclusive)"
                )
        ...
        if not isinstance(self.crossover_sensitivity, Decimal):
            try:
                ...
                raise ConfigurationError(
                    "crossover_sensitivity must be convertible to Decimal"
                )

        if self.crossover_sensitivity < 0 or self.crossover_sensitivity > Decimal(
            "0.1"
        ):
            raise ConfigurationError(
                "crossover_sensitivity must be between 0 and 0.1 (10%)"
            )
