...
class SMAIndicator(BaseIndicator):
    ...
    def generate_signal(self, current_result: IndicatorResult) -> Optional[Signal]:
        ...
        if (
            ...
            and current_price > sma_value
            and abs(current_price - sma_value) / sma_value
            > self.sma_config.crossover_sensitivity
        ):
        ...
        if signal_direction:
            confidence = min(
                Decimal("0.8"),
                Decimal("0.5") + abs(current_price - sma_value) / sma_value * 10,
            )
