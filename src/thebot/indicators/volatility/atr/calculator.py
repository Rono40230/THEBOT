...
class ATRCalculator:
    ...
    def __init__(self, config: ATRConfig):
        ...
        self._true_ranges: deque = deque(maxlen=config.period) if config.smoothing_method == "sma" else None
    ...
    def _calculate_ema_atr(self, true_range: Decimal) -> Optional[Decimal]:
        ...
        self._current_atr = self.alpha * true_range + (Decimal('1') - self.alpha) * self._current_atr
        return self._current_atr
    ...
    def get_volatility_percentile(self, periods: int = 20) -> Optional[Decimal]:
        ...
        recent_atrs = [point['atr'] for point in list(self._history)[-available_periods:] if point['atr'] is not None]
    ...
    def get_recent_trend(self, periods: int = 5) -> Optional[str]:
        ...
        recent_atrs = [point['atr'] for point in list(self._history)[-periods:] if point['atr'] is not None]
        if len(recent_atrs) < periods:
            return None
        
        # Pente simple
        first_half = sum(recent_atrs[:periods//2]) / (periods//2)
        second_half = sum(recent_atrs[periods//2:]) / (len(recent_atrs) - periods//2)
