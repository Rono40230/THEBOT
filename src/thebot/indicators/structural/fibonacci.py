...
class FibonacciLevel:
    ...
    def __init__(self, price: float, ratio: float, level_type: str, 
...
class FibonacciIndicator(BaseIndicator):
    ...
    def __init__(self, config: FibonacciConfig = None):
        ...
        # Swings détectés
        self._significant_swings: List[Tuple[datetime, float, float, str]] = []  # (time, high, low, direction)
    ...
    def add_data(self, market_data: MarketData) -> Optional[IndicatorResult]:
        ...
        result = IndicatorResult(
            ...
            metadata={
                ...
                'retracement_count': len([l for l in self._fibonacci_levels if l.level_type == 'retracement']),
                'extension_count': len([l for l in self._fibonacci_levels if l.level_type == 'extension']),
                'active_swings': len(self._significant_swings)
            }
        )
    ...
    def _detect_significant_swings(self):
        ...
        for i in range(5, len(recent_data) - 5):
            ...
            # Local high
            if all(current.high >= recent_data[j].high for j in range(i-5, i+6) if j != i):
                highs.append((current.timestamp, current.high))
            
            # Local low  
            if all(current.low <= recent_data[j].low for j in range(i-5, i+6) if j != i):
    ...
    def _create_significant_swings(self, highs: List[Tuple], lows: List[Tuple]):
        ...
        all_points = [(t, p, 'high') for t, p in highs] + [(t, p, 'low') for t, p in lows]
        ...
        for i in range(len(all_points) - 1):
            ...
            if current_point[2] != next_point[2]:  # Types différents (high -> low ou low -> high)
                ...
                if move_size >= self.config.min_swing_size:
                    ...
                    if current_point[2] == 'high' and next_point[2] == 'low':
                        # Mouvement baissier
                        swing = (current_point[0], current_point[1], next_point[1], 'bearish')
                    else:
                        # Mouvement haussier
                        swing = (current_point[0], next_point[1], current_point[1], 'bullish')
    ...
    def _calculate_fibonacci_levels(self):
        ...
        for swing_time, swing_high, swing_low, direction in self._significant_swings[-3:]:  # 3 derniers swings
    ...
    def _cleanup_old_levels(self):
        ...
        self._significant_swings = [
            swing for swing in self._significant_swings
            if swing[0] >= cutoff_time
        ]
    ...
    def generate_signal(self, current_result: IndicatorResult) -> Optional[Signal]:
        ...
        if distance <= 0.005:  # 0.5%
            ...
            is_key_level = any(abs(closest_level['ratio'] - key) < 0.01 for key in key_levels)
            ...
            if is_key_level:
                ...
                if closest_level['type'] == 'retracement':
                    ...
                    direction = SignalDirection.LONG if current_price < closest_level['price'] else SignalDirection.SHORT
                else:
            ...
            'retracement_levels': [
                {
                    ...
                    'line_dash': 'solid' if level.ratio in [0.382, 0.5, 0.618] else 'dash'
                } for level in self._fibonacci_levels 
                if level.level_type == 'retracement' and level.is_active
            ],
            'extension_levels': [
