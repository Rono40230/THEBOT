...
class PivotLevel:
    ...
    def __init__(
        self, price: float, level_type: str, level_name: str, calculation_date: datetime
    ):
        self.price = price
...
class PivotPointsIndicator(BaseIndicator):
    ...
    def __init__(self, config: PivotPointsConfig = None):
        ...
        self._daily_data: Dict[str, Dict] = (
            {}
        )  # Format: {'2024-01-01': {'high': x, 'low': y, 'close': z}}
    ...
    def add_data(self, market_data: MarketData) -> Optional[IndicatorResult]:
        ...
        result = IndicatorResult(
            ...
            metadata={
                ...
                "timeframe": self.config.timeframe.value,
                "calculation_date": (
                    self._last_calculation_date.isoformat()
                    if self._last_calculation_date
                    else None
                ),
            },
        )
    ...
    def _should_recalculate_pivots(self, current_date) -> bool:
        ...
        if self.config.timeframe == PivotTimeframe.DAILY:
            ...
            return (
                ...
                and current_date > self._last_calculation_date
            )
        elif self.config.timeframe == PivotTimeframe.MONTHLY:
            # Recalculer le 1er du mois
            return current_date.day == 1 and current_date > self._last_calculation_date
    ...
    def _calculate_demark_pivots(
        self, high: float, low: float, close: float, open_price: float
    ) -> Dict[str, float]:
            ...
            "pivot_levels": [
                {
                    ...
                    "line_width": (
                        3
                        if level.level_name == "PP"
                        else (2 if level.touches > 0 else 1)
                    ),
