...
class SRLevel:
    ...
    def __init__(self, price: float, level_type: str, strength: int = 1, 
                 first_touch: datetime = None, last_touch: datetime = None):
        self.price = price
...
class SupportResistanceIndicator(BaseIndicator):
    ...
    def current_value(self) -> Optional[Dict]:
        ...
        return {
            'support_levels': [
                ...
                } for level in self._support_levels if level.is_active
            ],
            'resistance_levels': [
                {
                    'price': level.price,
                    ...
                    'type': level.level_type,
                    'active': level.is_active
                } for level in self._resistance_levels if level.is_active
            ]
        }
    ...
    def add_data(self, market_data: MarketData) -> Optional[IndicatorResult]:
        ...
        result = IndicatorResult(
            ...
            metadata={
                ...
                'resistance_count': len([l for l in self._resistance_levels if l.is_active]),
                'total_levels': len(self._support_levels) + len(self._resistance_levels)
            }
        )
    ...
    def _group_and_create_levels(self, candidates: List[Tuple[float, datetime]], level_type: str):
        ...
        for i in range(1, len(candidates)):
            price_diff = abs(candidates[i][0] - current_group[-1][0]) / current_group[-1][0]
        ...
        # Créer les niveaux
        levels_list = self._resistance_levels if level_type == 'resistance' else self._support_levels
    ...
    def generate_signal(self, current_result: IndicatorResult) -> Optional[Signal]:
        ...
        for level in support_levels:
            if not closest_support or abs(current_price - level['price']) < abs(current_price - closest_support['price']):
                closest_support = level
                
        for level in resistance_levels:
            if not closest_resistance or abs(current_price - level['price']) < abs(current_price - closest_resistance['price']):
                closest_resistance = level
        
        # Générer signal selon proximité
        distance_threshold = current_price * 0.01  # 1%
        
        if closest_support and abs(current_price - closest_support['price']) <= distance_threshold:
            return Signal(
                ...
                metadata={'level_type': 'support', 'level_price': closest_support['price']}
            )
        
        if closest_resistance and abs(current_price - closest_resistance['price']) <= distance_threshold:
            return Signal(
                ...
                indicator_name=self.name,
                timestamp=datetime.now(),
                metadata={'level_type': 'resistance', 'level_price': closest_resistance['price']}
            )
