...
try:
    from PyQt6.QtWidgets import (
        ...
        QGroupBox, QGridLayout, QTextEdit, QProgressBar, QSystemTrayIcon,
        QMenu, QMenuBar, QStatusBar, QFrame, QScrollArea
    )
    from PyQt6.QtCore import QThread, pyqtSignal, QTimer, Qt, QSize
    from PyQt6.QtGui import QPixmap, QIcon, QFont, QPalette, QColor, QAction
    ...
    class THEBOTMainWindow(QMainWindow):
        ...
        def update_market_data(self):
            ...
            if len(self.price_history[current_symbol]) > 200:
                self.price_history[current_symbol] = self.price_history[current_symbol][-200:]
        ...
        def update_indicators(self, data: MarketData):
            ...
            atr_result = self.indicators['ATR'].add_ohlc(data.high, data.low, data.close)
        ...
        def update_display(self, data: MarketData):
            ...
            self.current_sma_label.setText(f"SMA({self.sma_period.value()}): {sma_val:.2f}" if sma_val else "SMA: --")
            self.current_ema_label.setText(f"EMA({self.ema_period.value()}): {ema_val:.2f}" if ema_val else "EMA: --")
            self.current_atr_label.setText(f"ATR({self.atr_period.value()}): {atr_val:.4f}" if atr_val else "ATR: --")
            self.current_rsi_label.setText(f"RSI({self.rsi_period.value()}): {rsi_val:.1f}" if rsi_val else "RSI: --")
        ...
        def check_signals(self, data, sma_result, ema_result, atr_result, rsi_result):
            ...
            if rsi_result and hasattr(rsi_result, 'signal'):
                if rsi_result.signal == 'OVERSOLD':
                    signals.append(f"🟢 RSI Survente: {rsi_result.value:.1f} < {self.rsi_oversold.value()}")
                elif rsi_result.signal == 'OVERBOUGHT':
                    signals.append(f"🔴 RSI Surachat: {rsi_result.value:.1f} > {self.rsi_overbought.value()}")
                    
            # Signaux croisement MA
            if sma_result and ema_result:
                sma_val = sma_result.value if hasattr(sma_result, 'value') else sma_result
                ema_val = ema_result.value if hasattr(ema_result, 'value') else ema_result
            ...
            if atr_result and hasattr(atr_result, 'volatility_percentile'):
                if atr_result.volatility_percentile > 80:
                    signals.append(f"⚡ Forte volatilité: {atr_result.volatility_percentile:.0f}%")
                ...
                for name, indicator in self.indicators.items():
                    try:
                        if name in ['SMA', 'EMA', 'RSI']:
                            ...
                            result = indicator.add_ohlc(test_price, test_price * Decimal('0.99'), test_price)
...
if not PYQT_AVAILABLE:
    ...
    class THEBOTTkinterApp:
        ...
        def setup_ui(self):
            ...
            # Titre
            title_label = ttk.Label(main_frame, text="THEBOT - Native Desktop App", 
                                  font=('Arial', 16, 'bold'))
            title_label.pack(pady=10)
            
            # Message d'information
            info_text = ttk.Label(main_frame, text="""
...
Architecture: Ultra-Modulaire validée
            ...
            ttk.Button(button_frame, text="🚀 Version PyQt6", 
                      command=self.suggest_pyqt).pack(side='left', padx=5)
            ttk.Button(button_frame, text="📊 Dashboard Jupyter", 
                      command=self.open_jupyter).pack(side='left', padx=5)
            ttk.Button(button_frame, text="❌ Quitter", 
                      command=self.root.quit).pack(side='left', padx=5)
                      
        def suggest_pyqt(self):
            messagebox.showinfo("Installation PyQt6", 
                              "Pour installer PyQt6:\\n\\npip install PyQt6\\n\\nPuis relancez l'application.")
                              
        def open_jupyter(self):
            messagebox.showinfo("Dashboard Jupyter", 
