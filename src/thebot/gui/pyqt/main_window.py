...
class IndicatorsPanel(QGroupBox):
        ...
        try:
            ...
            if results:
                ...
                self.results_text.append(
                    f"✅ SMA({period}): {avg:.4f} | Dernière valeur: {results[-1]:.4f}"
                )
    ...
    def calculate_ema(self):
        ...
        try:
            ...
            if results:
                self.results_text.append(
                    f"✅ EMA({period}): {results[-1]:.4f} | Trend: {results[-1] - results[-10] if len(results) > 10 else 0:.4f}"
                )
        ...
        try:
            ...
            if results:
                ...
                status = (
                    "🔴 Survente"
                    if current_rsi < 30
                    else "🟢 Surachat" if current_rsi > 70 else "🔵 Neutre"
                )
                self.results_text.append(
                    f"✅ RSI({period}): {current_rsi:.2f} | {status}"
                )
        ...
        try:
            ...
            if results:
                ...
                volatility = (
                    "📈 Haute"
                    if current_atr > 2
                    else "📊 Normale" if current_atr > 1 else "📉 Faible"
                )
                self.results_text.append(
                    f"✅ ATR({period}): {current_atr:.4f} | Volatilité: {volatility}"
                )
    ...
    def on_indicator_calculated(self, name: str, data):
        ...
        try:
            if len(data) > 0:
                ...
                self.chart_widget.axes.plot(
                    x, data, color=color, linewidth=2, label=f"{name}", alpha=0.8
                )
                self.chart_widget.axes.legend()
                self.chart_widget.draw()

                self.statusBar().showMessage(
                    f"✅ {name} calculé avec succès - {len(data)} points de données"
                )
        except Exception as e:
    ...
    def run_tests(self):
        ...
        try:
            result = subprocess.run(
                [sys.executable, "-m", "pytest", "tests/unit/indicators/", "-v"],
                capture_output=True,
                text=True,
                cwd="/home/rono/THEBOT",
            )

            if result.returncode == 0:
                self.indicators_panel.results_text.append(
