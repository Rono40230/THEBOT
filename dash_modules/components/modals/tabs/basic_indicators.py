...
class BasicIndicatorsTab:
    ...
    def _create_atr_section(self, atr_config: Dict) -> html.Div:
        ...
        controls = [
            ...
            dbc.Row([
                ...
                dbc.Col([
                    html.Div(id="atr-preview", className="mt-2")
                ], width=6)
            ])
        ]
        ...
        return dbc.Card([
            ...
            dbc.CardBody([
                dbc.Row([
                    dbc.Col([
                        dbc.Button(
                            [html.I(className="fas fa-toggle-on me-2"), "Tout Activer"],
                    ...
                    dbc.Col([
                        dbc.Button(
                            [html.I(className="fas fa-palette me-2"), "Couleurs Aléatoires"],
    ...
    def _register_callbacks(self):
        ...
        def update_previews(sma_period, ema_period, rsi_period, rsi_ob, rsi_os, 
                          atr_period, atr_mult, macd_fast, macd_slow, macd_signal):
            """Mettre à jour les previews en temps réel"""
            
            # SMA Preview
            sma_preview = dbc.Badge(
                f"SMA({sma_period or 20})",
                color="primary",
                className="me-2"
            )
            ...
            rsi_preview = html.Div([
                dbc.Badge(f"RSI({rsi_period or 14})", color="purple", className="me-2"),
            ...
            atr_preview = html.Div([
                dbc.Badge(f"ATR({atr_period or 14})", color="success", className="me-2"),
                dbc.Badge(f"x{atr_mult or 2.0}", color="info", className="me-1")
            ])
            
            # MACD Preview
            macd_preview = html.Div([
                dbc.Badge(f"MACD({macd_fast or 12}, {macd_slow or 26})", color="primary", className="me-2"),
                dbc.Badge(f"Signal({macd_signal or 9})", color="secondary", className="me-1")
            ])
        ...
        def update_status(sma_enabled, ema_enabled, rsi_enabled, atr_enabled, macd_enabled):
