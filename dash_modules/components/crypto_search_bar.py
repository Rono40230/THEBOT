...
class CryptoSearchBar:
    ...
    def create_search_dropdown(self) -> dcc.Dropdown:
        ...
        return dcc.Dropdown(
            ...
            options=[
                {"label": symbol, "value": symbol} for symbol in self.initial_symbols
            ],
            value="BTCUSDT",
                    ...
                    [
                        dbc.Row(
                            [
                                ...
                                dbc.Col([self.create_timeframe_selector()], width=3),
                                dbc.Col([self.create_action_buttons()], width=5),
                            ],
                            align="start",
                        )  # Changé de "end" à "start" pour l'alignement
                    ]
                )
            ],
            className="mb-3",
    ...
    def get_symbol_options(self, search_value: str = None) -> List[Dict[str, str]]:
        ...
        if BINANCE_AVAILABLE and binance_provider:
            try:
                filtered_symbols = binance_provider.search_symbols(
                    search_value, limit=20
                )
                return [
                    {"label": symbol, "value": symbol} for symbol in filtered_symbols
                ]
            except Exception as e:
                self.logger.warning(f"Erreur recherche Binance: {e}")
