...
class TopPerformersComponent:
            ...
            for symbol1 in price_changes:
                ...
                for symbol2 in price_changes:
                    if symbol1 == symbol2:
                        ...
                        if (change1 > 0 and change2 > 0) or (
                            change1 < 0 and change2 < 0
                        ):
    ...
    def get_performance_ratios(self) -> Dict:
        ...
        try:
            ...
            ratios = {
                ...
                "strength_index": self._calculate_strength_index(
    ...
    def _identify_market_leaders(self, symbols_data: Dict) -> List[str]:
        ...
        for symbol, data in symbols_data.items():
            if (
                data["volume"] > 20000000 and abs(data["change_24h"]) < 5
            ):  # Stabilité avec volume
...
def update_top_performers(mode, limit, n_intervals):
    ...
    try:
        ...
        corr_fig.update_layout(title="Correlation Matrix", height=400)
            ...
            ratios_content = [
                html.Div(
                    [
                        ...
                        html.P(
                            f"Market Condition: {ratios.get('market_condition', 'Unknown')}"
                        ),
                        html.P(
                            f"Bull/Bear Ratio: {ratios.get('bull_bear_ratio', 0):.2f}"
                        ),
                        html.P(
                            f"Strength Index: {ratios.get('strength_index', 0):.1f}%"
                        ),
                        html.P(
                            f"Market Momentum: {ratios.get('market_momentum', 0):.1f}%"
                        ),
                    ...
                    + [
                        html.Tr(
                            [
                                ...
                                html.Td(
                                    format_percentage_change(l["change_percent"]),
                                    style={"color": "red"},
                                ),
