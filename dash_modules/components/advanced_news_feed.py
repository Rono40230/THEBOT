...
class AdvancedNewsFeed:
            ...
            [
                ...
                dbc.Row(
                    [
                        ...
                        dbc.Col([self.create_news_sidebar()], width=4),
                    ]
                ),
        ...
        return html.Div(
            [
                ...
                dbc.Card(
                    [
                        dbc.CardHeader(
                            [html.H5([html.I(className="fas fa-rss me-2"), "Sources"])]
                        ),
                        dbc.CardBody(
                            [
                                html.Div(
    ...
    def create_article_modal(self) -> dbc.Modal:
        ...
        return dbc.Modal(
            [
                dbc.ModalHeader([dbc.ModalTitle(id="article-modal-title")]),
                dbc.ModalBody(
                    [
                        # Métadonnées article
                ...
                dbc.ModalFooter(
                    [dbc.Button("Fermer", id="article-modal-close", color="secondary")]
                ),
            ],
            id="article-modal",
            size="lg",
    ...
    def format_news_articles(
        self, articles: List[Dict], filters: Dict
    ) -> List[html.Div]:
            ...
            return [
                html.Div(
                    [
                        ...
                        html.P(
                            "Essayez de modifier vos filtres ou d'actualiser.",
        ...
        for i, article in enumerate(articles):
            # Métadonnées
            source = article.get(
                "rss_source_name", article.get("source", "Source inconnue")
            )
            ...
            # Badges
            source_info = self.news_sources.get(
                source, {"color": "secondary", "reliability": "unknown"}
            )
            category_info = self.news_categories.get(
                category, {"color": "secondary", "icon": "📄"}
            )
                                            ...
                                            html.Div(
                                                [
                                                    dbc.Badge(
                                                        [
                                                            category_info["icon"],
                                                            f" {category_info.get('label', category)}",
                                                        ],
    ...
    def filter_articles(self, articles: List[Dict], filters: Dict) -> List[Dict]:
        ...
        if filters.get("source") and filters["source"] != "all":
            filtered = [
                a
                for a in filtered
                if a.get("rss_source_name") == filters["source"]
                or a.get("source") == filters["source"]
            ]
