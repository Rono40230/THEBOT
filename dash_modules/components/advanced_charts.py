...
class AdvancedCharts:
    ...
    def __init__(self):
        self.chart_types = {
            'candlestick': {'label': '🕯️ Chandelier', 'description': 'Graphique en chandeliers'},
        ...
        self.indicators = {
            'sma': {'label': 'SMA', 'description': 'Moyenne Mobile Simple', 'params': ['period']},
            'ema': {'label': 'EMA', 'description': 'Moyenne Mobile Exponentielle', 'params': ['period']},
            'bollinger': {'label': 'Bollinger', 'description': 'Bandes de Bollinger', 'params': ['period', 'std']},
            'rsi': {'label': 'RSI', 'description': 'Relative Strength Index', 'params': ['period']},
            'macd': {'label': 'MACD', 'description': 'MACD Convergence Divergence', 'params': ['fast', 'slow', 'signal']},
            'volume_sma': {'label': 'Volume SMA', 'description': 'Moyenne Mobile du Volume', 'params': ['period']},
            'support_resistance': {'label': 'S/R', 'description': 'Support et Résistance', 'params': ['lookback']}
        }
        ...
        self.color_schemes = {
            'default': {
                ...
                'bg': '#ffffff', 'grid': '#f0f0f0'
            },
            'dark': {
                'up': '#4caf50', 'down': '#f44336',
                'volume_up': '#4caf50', 'volume_down': '#f44336',
                'bg': '#2e2e2e', 'grid': '#404040'
            },
            'crypto': {
                'up': '#f7931a', 'down': '#8b5cf6',
                'volume_up': '#f7931a', 'volume_down': '#8b5cf6',
                'bg': '#ffffff', 'grid': '#f8f9fa'
            }
        }
                ...
                dbc.Row([
                    ...
                    dbc.Col([
                        ...
                        dbc.Select(
                            ...
                            options=[
                                {"label": info['label'], "value": chart_type}
                    ...
                    dbc.Col([
                        ...
                        dbc.Checklist(
                            ...
                            options=[
                                {"label": info['label'], "value": indicator}
    ...
    def create_chart_settings_panel(self) -> dbc.Offcanvas:
        ...
        ], id="chart-settings-offcanvas", title="Paramètres", is_open=False)
    ...
    def create_candlestick_chart(self, data: pd.DataFrame, config: Dict) -> go.Figure:
        ...
        fig = make_subplots(
            ...
            row_heights=[0.6] + [0.2] * (subplot_rows - 1) if subplot_rows > 1 else [1.0]
        )
        ...
        # Volume
        if 'show_volume' in config.get('display_options', []) and current_row <= subplot_rows:
            colors = [theme['volume_up'] if close >= open_price else theme['volume_down'] 
