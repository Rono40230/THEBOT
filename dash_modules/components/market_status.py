...
class MarketStatusManager:
    ...
    def get_market_status_badge(self, market_code: str) -> dbc.Badge:
        ...
        return dbc.Badge(f"{market_code}: {status_text}", color=color, className="me-2")
    ...
    def get_market_status_with_refresh(
        self, component_id: str = "market-status-container"
    ) -> html.Div:
        """Composant avec mise à jour automatique (nécessite un callback)"""
        return html.Div([self.get_market_status_component()], id=component_id)
