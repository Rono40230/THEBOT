...
class StocksModule(BaseMarketModule):
    ...
    def __init__(self, calculators: Dict = None):
        # Get stocks provider from new config system
        from ..core.config_manager import get_global_config
        config = get_global_config()

        # Use Twelve Data for stocks if enabled, otherwise Yahoo Finance
        if config.get("providers.twelve_data.enabled"):
            stocks_provider = "twelve_data"
            api_key = config.get("providers.twelve_data.api_key", "")
        else:
            stocks_provider = "yahoo_finance"
                ...
                dbc.Tabs(
                    [
                        dbc.Tab(
                            ...
                            tab_id="ai-tab",
                            children=[
                                html.Div([self.create_ai_dashboard()], className="p-3")
                            ],
                        )
                    ],
                    id="secondary-tabs",
                            ...
                            [
                                ...
                                dcc.Dropdown(
                                    ...
                                    options=[
                                        ...
                                        {
                                            "label": "⚡ 5m - Quick Trades",
                                            "value": "5m",
                                        },
                                        {
                                            "label": "📊 15m - Short Term",
                                            "value": "15m",
                                        },
    ...
    def _get_valuation_metrics(self, symbol: str) -> Dict:
        ...
        return {
            ...
            "valuation": (
                "Fair"
                if 15 <= estimated_pe <= 25
                else ("Expensive" if estimated_pe > 25 else "Cheap")
            ),
        }
    ...
    def create_stocks_ai_dashboard(self) -> dict:
        ...
        return {
            ...
            "key_levels": {"support": "$165.20", "resistance": "$182.50"},
            "earnings_calendar": [
                {
                    "date": "2024-01-15",
                    "company": "Major Bank Earnings",
                    "impact": "High",
                },
                {
                    "date": "2024-01-18",
                    "company": "Tech Earnings Season",
                    "impact": "High",
                },
            ],
            "analyst_ratings": {
                                            ...
                                            [
                                                html.Div(
                                                    ...
                                                    children=[
                                                        ...
                                                        html.P(
                                                            "Investment recommendations loading...",
                                                            className="text-muted",
                                                        ),
                                                    ],
                                                )
                        ...
                        dbc.CardBody(
                            [
                                html.Div(
                                    ...
                                    children=[
                                        html.P(
                                            "🔮 L'IA locale analyse le marché des actions...",
                                            className="text-info",
                                        ),
                                        html.P(
                                        ...
                                        html.P(
                                            "💡 Recommandations de trading en génération...",
    ...
    def setup_callbacks(self, app):
        ...
        @app.callback(
            ...
            [Input("ai-insights-btn", "n_clicks"), Input("symbol-dropdown", "value")],
            [State("ai-model", "value"), State("ai-enabled", "value")],
        )
        def update_stocks_ai_analysis(n_clicks, symbol, ai_model, ai_enabled):
            ...
            if not ai_enabled or not symbol:
                return (
                    ...
                    html.P("IA désactivée", className="text-muted"),
                    html.P(
                        "💤 IA locale en veille. Activez l'analyse IA pour obtenir des insights.",
                        className="text-info",
                    ),
                )
            ...
            try:
                ...
                sentiment_result = local_ai_engine.analyze_sentiment(
                    news_articles, market_type="stocks"
                )
                technical_result = local_ai_engine.analyze_technical_pattern_simple(
                ...
                sentiment_display = html.Div(
                    [
                        ...
                        html.P(
                            f"Confiance: {sentiment_result['confidence']:.1f}%",
                            className="text-muted",
                        ),
                        html.Small(
                            f"Earnings focus: {sentiment_result['analysis']['bullish_articles']} positifs"
                        ),
                    ]
                )
                ...
                technical_display = html.Div(
                    [
                        ...
                        html.P(
                            f"Support: ${price_data[-1]:.2f}",
                            className="small text-success",
                        ),
                        html.Small(
                            " | ".join(
                                technical_result.get("signals", ["Analyse en cours"])
                            )
                        ),
                    ]
                )
                ...
                trading_display = html.Div(
                    [
                        html.H4(
                            trading_insight["action"],
                            className=f"text-{'success' if trading_insight['action'] == 'BUY' else 'danger' if trading_insight['action'] == 'SELL' else 'warning'}",
                        ),
                        html.P(
                            trading_insight["explanation"][:50] + "...",
                            className="small",
                        ),
