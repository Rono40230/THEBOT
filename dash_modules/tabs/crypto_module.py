...
from dash_modules.core.price_formatter import (
    ...
    format_price_label_adaptive,
    format_volume_adaptive,
)
...
# Import du modal IA
try:
    from ..components.ai_trading_modal import (
        ai_trading_modal,
        register_ai_modal_callbacks,
    )
...
try:
    from ..components.price_alerts_modal import (
        alerts_store,
        price_alerts_modal,
        register_alerts_modal_callbacks,
    )
...
try:
    from ..components.crypto_chart_components import (
        CryptoChartComponents,
        crypto_chart_components,
    )
    from ..components.crypto_search_bar import CryptoSearchBar, crypto_search_bar
    from ..components.technical_indicators import (
        TechnicalIndicators,
...
try:
    from dash_modules.core.calculators import (
        FairValueGapCalculator,
...
class CryptoModule:
    ...
    def initialize_default_symbol(self) -> None:
    ...
    def create_candlestick_chart(
        ...
        try:
            ...
            logger.error(f"Erreur création candlestick: {e}")
            return go.Figure()
    ...
    def create_volume_chart(self, df: pd.DataFrame) -> go.Figure:
        ...
        try:
            ...
            colors = [
                "#00d4aa" if df["close"].iloc[i] >= df["open"].iloc[i] else "#ff6b6b"
            ...
            fig.update_layout(
                ...
                showlegend=False,
                xaxis=dict(
                    showticklabels=False, tickformat=""  # Cache les labels des dates
                ),
            )
            ...
            logger.error(f"Erreur création volume: {e}")
            return go.Figure()
    ...
    def create_technical_indicators_chart(self, df):
        ...
        try:
            ...
            if len(df) >= 14:
                ...
                fig.add_trace(
                    go.Scatter(
                        x=df.index, y=rsi, name="RSI", line=dict(color="#ffd93d")
                    ),
                    row=1,
                    col=1,
                )
                ...
                fig.add_hline(
                    y=70, row=1, col=1, line_dash="dash", line_color="red", opacity=0.5
                )
                fig.add_hline(
                    y=30,
                    row=1,
            ...
            if len(df) >= 26:
                ...
                fig.add_trace(
                    go.Scatter(
                        x=df.index, y=macd_line, name="MACD", line=dict(color="#00d4aa")
                    ),
                    row=2,
                    col=1,
                )
            ...
            logger.error(f"Erreur création indicateurs: {e}")
            return go.Figure()
    ...
    def create_rsi_chart(self, df: pd.DataFrame) -> go.Figure:
        ...
        try:
            ...
            fig.add_trace(
                go.Scatter(
                    x=df.index, y=rsi, name="RSI", line=dict(color="#ffd93d", width=2)
                )
            )
            ...
            logger.error(f"Erreur création RSI: {e}")
            return go.Figure()
    ...
    def create_atr_chart(self, df):
        ...
        try:
            ...
            logger.error(f"Erreur création ATR: {e}")
            return go.Figure()
    ...
    def create_macd_chart(self, df):
        ...
        try:
            ...
            logger.error(f"Erreur création MACD: {e}")
            return go.Figure()
                                    ...
                                    [
                                        ...
                                        html.Div(
                                            [
                                                ...
                                                html.Span(
                                                    "Vol: ", className="text-muted"
                                                ),
                                                html.Span(
                                                    "",
                                                    id="crypto-volume-display",
                                                    className="fw-bold",
                                                ),
                                            ],
    ...
    def add_hidden_dropdowns_for_modal(self):
        ...
        return html.Div(
            [
                ...
                dcc.Dropdown(
                    id="crypto-timeframe-modal-sync", style={"display": "none"}
                ),
            ]
        )
    ...
    def get_layout(self):
        ...
        layout_components = [
            ...
            dbc.Container(
                [self.create_search_component()], fluid=True, className="mb-3"
            ),
            # Graphique principal en pleine largeur
            dbc.Container(
                [dbc.Row([dbc.Col([self.create_main_chart()], width=12)])],
                fluid=True,
                className="mb-3",
            ),
            # Graphiques secondaires
            dbc.Container(
                [self.create_secondary_charts()], fluid=True, className="mb-3"
            ),
        ]
