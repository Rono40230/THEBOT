...
class CryptoModule:
    ...
    def create_enhanced_price_display(self):
        """Crée la barre de contrôle complète avec prix, recherche, timeframe et boutons - VERSION MODULAIRE"""
        if MODULAR_COMPONENTS_AVAILABLE:
            # Utiliser le composant modulaire
            search_bar = crypto_search_bar.create_complete_search_bar()
            
            # Ajouter l'affichage du prix
            price_display = dbc.Card([
                dbc.CardBody([
                    dbc.Row([
                        dbc.Col([
                            html.Span(
            ...
            return html.Div([search_bar, price_display])
        else:
            # Version de secours - code original
            return self._create_enhanced_price_display_fallback()

    def _create_enhanced_price_display_fallback(self):
        """Version de secours de la barre de contrôle"""
        return dbc.Card([
            dbc.CardBody([
    ...
    def create_main_chart(self):
        """Crée le graphique principal avec candlesticks et prix en direct - VERSION MODULAIRE"""
        if MODULAR_COMPONENTS_AVAILABLE:
    ...
    def create_secondary_charts(self):
        """Crée les 3 graphiques secondaires (RSI, ATR, MACD) - VERSION MODULAIRE"""
        if MODULAR_COMPONENTS_AVAILABLE:
    ...
    def create_ai_insights_cards(self):
        """Crée les 3 cartes AI Insights - VERSION MODULAIRE"""
        if MODULAR_COMPONENTS_AVAILABLE:
            ...
            return dbc.Row([
                dbc.Col([
                    dbc.Card([
                        ...
                        dbc.CardBody([
                            html.P("Analyse en cours...", className="text-center text-muted")
                        ])
                    ])
                ], width=12)
            ], className="mb-4")
    ...
    def setup_callbacks(self, app):
        ...
        def update_main_chart(symbol, timeframe, sma_enabled, sma_period, ema_enabled, ema_period, 
            ...
            try:
                ...
                # === INDICATEURS STRUCTURELS VIA MODULE MODULAIRE ===
                # Calculer et ajouter les indicateurs structurels
                try:
                    # Configuration des indicateurs via le module modulaire
                    indicators_config = {
                        'sr_enabled': sr_enabled,
                        'sr_strength': sr_strength or 2,
                        'sr_lookback': sr_lookback or 50,
                        'sr_support_color': sr_support_color or '#27AE60',
                        'sr_resistance_color': sr_resistance_color or '#E74C3C',
                        'sr_line_style': sr_line_style or 'solid',
                        'sr_line_width': sr_line_width or 2,
                        'fibonacci_enabled': fibonacci_enabled,
                        'fibonacci_swing': fibonacci_swing or 20,
                        'fibonacci_line_style': fibonacci_line_style or 'dashed',
                        'fibonacci_line_width': fibonacci_line_width or 1,
                        'fibonacci_transparency': fibonacci_transparency or 0.8,
                        'pivot_enabled': pivot_enabled,
                        'pivot_method': pivot_method or 'standard',
                        'pivot_line_style': pivot_line_style or 'dot',
                        'pivot_line_width': pivot_line_width or 2
                    }
                    
                    # Calcul via le module modulaire
                    if MODULAR_COMPONENTS_AVAILABLE:
                        structural_data = technical_indicators.calculate_all_indicators(data, indicators_config)
                        
                        # Ajouter les niveaux au graphique via le module modulaire
                        fig = crypto_chart_components.add_structural_levels_to_chart(fig, structural_data)
                        ...
                        if active_indicators:
                            fig.add_annotation(
                                text=f"📊 Modulaire: {', '.join(active_indicators)}",
                                xref="paper", yref="paper",
                                x=0.02, y=0.98,
                                showarrow=False,
                    ...
                    print(f"⚠️ Erreur indicateurs modulaires: {e}")
    ...
    # === INDICATEURS TECHNIQUES - VERSION MODULAIRE ===
    
    def calculate_indicators(self, data: pd.DataFrame, config: dict) -> dict:
        """Calcule tous les indicateurs techniques via le module modulaire"""
        if MODULAR_COMPONENTS_AVAILABLE:
            return technical_indicators.calculate_all_indicators(data, config)
        else:
            # Version de secours avec indicateurs de base
            return self._calculate_indicators_fallback(data, config)
    
    def _calculate_indicators_fallback(self, data: pd.DataFrame, config: dict) -> dict:
        """Version de secours pour les indicateurs"""
        results = {}
        
        try:
            if config.get('rsi_enabled', False):
                results['rsi'] = self._calculate_rsi_fallback(data['close'], config.get('rsi_period', 14))
            
            if config.get('atr_enabled', False):
                results['atr'] = self._calculate_atr_fallback(data, config.get('atr_period', 14))
            
            if config.get('macd_enabled', False):
                results['macd'] = self._calculate_macd_fallback(
                    data['close'], 
            ...
            print(f"⚠️ Erreur calcul indicateurs fallback: {e}")
        
        return results
    
    def _calculate_rsi_fallback(self, prices, period=14):
        """Version de secours RSI"""
        try:
            delta = prices.diff()
    ...
    def _calculate_atr_fallback(self, data, period=14):
        """Version de secours ATR"""
        try:
            high_low = data['high'] - data['low']
    ...
    # === MÉTHODES DÉPLACÉES VERS TECHNICAL_INDICATORS MODULE ===
    # Les méthodes suivantes ont été déplacées vers dash_modules.components.technical_indicators:
    # - calculate_rsi() -> technical_indicators.calculate_rsi()
    # - calculate_atr() -> technical_indicators.calculate_atr()
    # - calculate_atr_signals() -> technical_indicators.calculate_atr_signals()
    # - calculate_macd() -> technical_indicators.calculate_macd()
    # - calculate_support_resistance_simple() -> technical_indicators.calculate_support_resistance()
    # - calculate_fibonacci_simple() -> technical_indicators.calculate_fibonacci_retracements()
    # - calculate_pivot_points_simple() -> technical_indicators.calculate_pivot_points()
    # - calculate_structural_indicators() -> technical_indicators.calculate_all_indicators()
    # - add_structural_levels_to_chart() -> crypto_chart_components.add_structural_levels_to_chart()

    # === CALLBACKS ET MÉTHODES PRINCIPALES ===

    # === MÉTHODES STRUCTURELLES - VERSION MODULAIRE ===
    
    def calculate_structural_indicators(self, data, **kwargs):
        """Calcule les indicateurs structurels via le module modulaire"""
        if MODULAR_COMPONENTS_AVAILABLE:
            # Convertir les paramètres vers le format du module
            config = {
                'sr_enabled': kwargs.get('sr_enabled', False),
                'sr_strength': kwargs.get('sr_strength', 2),
                'fibonacci_enabled': kwargs.get('fibonacci_enabled', False),
                'fibonacci_swing': kwargs.get('fibonacci_swing', 2),
                'pivot_enabled': kwargs.get('pivot_enabled', False),
                'pivot_method': kwargs.get('pivot_method', 'standard'),
            }
            
            # Ajouter les paramètres visuels
            visual_params = kwargs.get('visual_params', {})
            if visual_params:
    ...
    def add_structural_levels_to_chart(self, fig, structural_data, visual_params=None):
        """Ajoute les niveaux structurels au graphique via le module modulaire"""
        if MODULAR_COMPONENTS_AVAILABLE:
            return crypto_chart_components.add_structural_levels_to_chart(fig, structural_data)
        else:
            # Version de secours - retourner la figure sans modification
            return fig
