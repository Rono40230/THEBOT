...
class BaseNewsModule(BaseMarketModule):
    ...
    def load_news_data(self, category: str = 'All News', limit: int = 50) -> pd.DataFrame:
        ...
        try:
            ...
            if filtered_news and len(filtered_news) > 0:
                ...
                print(f"✅ {self.news_type}: {len(news_data)} news articles loaded from filtered providers")
                return news_data
            else:
    ...
    def _filter_by_time_range(self, data: pd.DataFrame, time_range: str) -> pd.DataFrame:
        ...
        try:
            ...
            print(f"📅 Time filter ({time_range}): {len(data)} → {len(filtered_data)} articles")
    ...
    def create_news_feed(self, news_data: pd.DataFrame, sentiment_filter: str = 'all') -> tuple:
        ...
        if news_data.empty:
            return html.Div([
                html.P(f"Aucune actualité {self.news_type} disponible pour le moment.", 
        ...
        for idx, article in news_data.iterrows():
            # Calculate time ago
            time_ago = self._calculate_time_ago(article.get('published_at', article.get('time_published', '')))
            ...
            articles_data.append({
                ...
                'published_at': article.get('published_at', article.get('time_published', '')),
        ...
        if sentiment_filter != 'all':
            articles_data = [a for a in articles_data if a.get('sentiment', 'neutral') == sentiment_filter]
        ...
        for idx, article in enumerate(articles_data):
            ...
            sentiment_badge = dbc.Badge([
                sentiment_icons.get(sentiment, '😐'),
                f" {sentiment.title()}"
            ], color=sentiment_colors.get(sentiment, 'secondary'), className="me-2")
    ...
    def create_market_impact_widget(self, news_data: pd.DataFrame) -> html.Div:
        ...
        if news_data.empty:
            return html.Div("Pas de données d'impact disponibles", className="text-muted")
        ...
        try:
            ...
            return html.Div([
                ...
                html.Div([
                    dbc.Row([
                        dbc.Col([
                            html.H4(str(total_articles), className="text-primary mb-0"),
                    ...
                    dbc.Row([
                        dbc.Col([
                            ...
                            html.Div([
                                dbc.Progress(value=sentiment_counts.get('positive', 0), 
                                           color="success", className="mb-1",
                                           label=f"Positif: {sentiment_counts.get('positive', 0)}"),
                                dbc.Progress(value=sentiment_counts.get('neutral', 0), 
                                           color="info", className="mb-1",
                                           label=f"Neutre: {sentiment_counts.get('neutral', 0)}"),
                                dbc.Progress(value=sentiment_counts.get('negative', 0), 
    ...
    def create_economic_calendar_widget(self) -> html.Div:
        ...
        return html.Div([
            ...
            html.Div([
                html.P("📅 Événements économiques à venir", className="text-muted"),
                html.Small("Fonctionnalité en développement...", className="text-muted")
            ])
        ])
    ...
    def translate_to_french(self, text: str) -> str:
        ...
        try:
            ...
            french_words = ['le', 'la', 'les', 'et', 'est', 'une', 'des', 'pour', 'dans', 'avec']
