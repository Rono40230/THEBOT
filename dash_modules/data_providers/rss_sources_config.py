...
class RSSSourcesConfig:
    ...
    def get_active_sources(self, category: str = None) -> List[Dict[str, Any]]:
        ...
        if category:
            return [source for source in self.sources.get(category, []) if source.get('active', False)]
        
        active_sources = []
        for cat_sources in self.sources.values():
            active_sources.extend([source for source in cat_sources if source.get('active', False)])
