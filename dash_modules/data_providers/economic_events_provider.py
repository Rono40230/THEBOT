...
from .economic_events_config import economic_events_config, EconomicEvent, EventImpact, EventCategory
...
class EconomicEventsProvider:
    ...
    def _generate_realistic_forecast(self, event_id: str, base_value: float) -> Tuple[float, float]:
        ...
        if random.random() < surprise_chance:
            # Surprise importante
            surprise_factor = random.uniform(1.5, 3.0) if random.random() > 0.5 else random.uniform(0.5, 0.8)
    ...
    def get_events_for_period(self, start_date: datetime, end_date: datetime, 
                             selected_events: List[str] = None) -> List[Dict]:
        ...
        if selected_events:
            events_to_process = [e for e in events_to_process if e.id in selected_events]
    ...
    def _should_include_event(self, event: EconomicEvent, date: datetime) -> bool:
        ...
        if event.frequency.value == "weekly":
            ...
            if event.id == "us_nfp":
                ...
                first_friday = self._get_first_weekday_of_month(date.year, date.month, 4)  # 4 = vendredi
    ...
    def _get_first_weekday_of_month(self, year: int, month: int, weekday: int) -> datetime:
    ...
    def _generate_event_data(self, event: EconomicEvent, date: datetime) -> Dict:
        ...
        if event.id in self.historical_data:
            ...
            if is_future:
                ...
                forecast_value, _ = self._generate_realistic_forecast(event.id, base_value)
                previous_value = base_value
            else:
                # Événement passé - générer prévision et valeur réelle
                forecast_value, actual_value = self._generate_realistic_forecast(event.id, base_value)
                previous_value = base_value
        ...
        market_impact = self._calculate_market_impact(event, forecast_value, actual_value)
    ...
    def _calculate_market_impact(self, event: EconomicEvent, forecast: Optional[float], 
        ...
        if forecast is not None and actual is not None:
            ...
            impact_data['actual_volatility'] = impact_data['expected_volatility'] * (1 + surprise_ratio)
            
            # Direction basée sur surprise positive/négative
            if actual > forecast:
                impact_data['direction'] = 'positive' if event.id not in ['us_unemployment_rate', 'initial_jobless_claims'] else 'negative'
            elif actual < forecast:
                impact_data['direction'] = 'negative' if event.id not in ['us_unemployment_rate', 'initial_jobless_claims'] else 'positive'
    ...
    def get_upcoming_events(self, days_ahead: int = 7, selected_events: List[str] = None) -> List[Dict]:
    ...
    def get_recent_events(self, days_back: int = 7, selected_events: List[str] = None) -> List[Dict]:
    ...
    def search_events(self, query: str, days_range: int = 30) -> List[Dict]:
        ...
        for event in events:
            if (query_lower in event['name'].lower() or 
                ...
                query_lower in event['country'].lower()):
                filtered_events.append(event)
        
        return filtered_events
    
    def get_calendar_data(self, month: int, year: int, selected_events: List[str] = None) -> Dict:
