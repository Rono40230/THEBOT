...
class RSSNewsManager(NewsProviderInterface):
    ...
    def get_news(
        ...
        try:
            logger.info(
                f"📰 Getting RSS news - Categories: {categories}, Sources: {sources}, Limit: {limit}"
            )
            ...
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                future_to_source = {
                    executor.submit(
                        self._fetch_source_articles, source, use_cache
                    ): source
            ...
            logger.info(
                f"✅ Retrieved {len(result)} unique articles from {len(target_sources)} RSS sources"
            )
            return result
    ...
    def _determine_target_sources(
        self, categories: List[str], sources: List[str]
    ) -> List[Dict[str, Any]]:
        ...
        if sources:
            ...
            return [
                ...
                if s["name"] in sources and s.get("active", False)
            ]
    ...
    def _fetch_source_articles(
        self, source: Dict[str, Any], use_cache: bool = True
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if use_cache and articles:
                cache_duration = source.get(
                    "update_interval", self.default_cache_duration
                )
    ...
    def _put_in_cache(
        self, key: str, articles: List[Dict[str, Any]], duration_seconds: int
    ):
    ...
    def test_sources(self, categories: List[str] = None) -> Dict[str, Any]:
        ...
        logger.info(
            f"✅ RSS sources test completed: {results['successful']}/{results['total_sources']} successful"
        )
        return results

    def get_symbol_specific_news(
        ...
        try:
            ...
            if any(
                ...
                for crypto in ["BTC", "ETH", "BNB", "ADA", "XRP", "SOL", "DOGE"]
            ):
                categories = ["crypto"]
            ...
            # Filtrer par symbole
            symbol_base = (
                symbol.replace("USDT", "").replace("USD", "").replace("EUR", "").upper()
            )
