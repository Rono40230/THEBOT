            ...
            for symbol in symbols:
                ...
                if not hist.empty:
                    ...
                    quotes[symbol] = {
                        ...
                        "volume": (
                            hist["Volume"].iloc[-1] if not hist["Volume"].empty else 0
                        ),
                ...
                df_data.append(
                    {
                        ...
                        "close": (
                            indicators["close"][i] if indicators["close"][i] else 0
                        ),
                        "volume": (
                            indicators["volume"][i]
                            if indicators.get("volume") and indicators["volume"][i]
                            else 0
                        ),
                    }
                )
        ...
        try:
            financial_topics = [
                (
                    ...
                    "Central bank monetary policy continues to influence market dynamics and economic outlook.",
                ),
                (
                    "Market Volatility",
                    "Financial markets show continued activity as investors assess economic conditions and corporate earnings.",
                ),
                (
                    "Economic Indicators",
                    "Key economic metrics provide insights into market trends and future economic direction.",
                ),
                (
                    "Corporate Earnings",
                    "Companies continue reporting quarterly results, influencing sector performance and market sentiment.",
                ),
                (
                    "Interest Rates",
                    "Interest rate environment remains a key factor in investment decisions and market valuations.",
                ),
            ]
                        ...
                        if abs(change_pct) > 0.8:
                            ...
                            direction = (
                                ...
                                if change_pct > 1.5
                                    ...
                                    "rose"
                                    if change_pct > 0
                                    else "fell" if change_pct > -1.5 else "dropped"
                                )
                            )
                            ...
                            news_item = {
                                ...
                                "summary": summary,
                                "url": f"https://finance.yahoo.com/quote/{symbol}",
                                "time_published": datetime.now().strftime(
                                    "%Y%m%dT%H%M%S"
                                ),
                                "published_at": datetime.now().isoformat(),
                ...
                try:
                    ...
                    if sector_changes and len(sector_changes) >= 1:
                        ...
                        if abs(avg_change) > 1.0:
                            ...
                            direction = (
                                "outperformed" if avg_change > 0 else "underperformed"
                            )
            ...
            for symbol in all_symbols[: min(3, limit)]:
                try:
                    ...
                    if not hist.empty and len(hist) >= 2:
                        ...
                        avg_volume = (
                            hist["Volume"].mean() if len(hist) > 1 else current_volume
                        )
                        ...
                        if news_events:
                            ...
                            news_item = {
                                ...
                                "sentiment": (
                                    "positive"
                                    if change_pct > 0
                                    else "negative" if change_pct < 0 else "neutral"
                                ),
