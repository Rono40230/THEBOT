...
class FinnhubEconomicCalendar:
    ...
    def __init__(self, api_key: str = None):
        ...
        self.category_keywords = {
            'monetary_policy': [
                'interest rate', 'fed', 'central bank', 'monetary policy',
                'rate decision', 'fomc', 'ecb', 'boe', 'boj', 'rba',
                'policy rate', 'discount rate', 'repo rate'
            ],
            'inflation': [
                'cpi', 'ppi', 'inflation', 'price index', 'deflator',
                'core inflation', 'headline inflation', 'pce'
            ],
            'employment': [
                'unemployment', 'jobs', 'employment', 'payroll', 'jobless',
                'labor', 'labour', 'workforce', 'hiring', 'layoffs'
            ],
            'gdp_growth': [
                'gdp', 'growth', 'economic output', 'quarterly growth',
                'real gdp', 'nominal gdp', 'economic expansion'
            ],
            'industrial': [
                'industrial production', 'manufacturing', 'factory',
                'production index', 'capacity utilization', 'ism'
            ],
            'retail': [
                'retail sales', 'consumer spending', 'sales data',
                'retail figures', 'consumption', 'spending'
            ],
            'housing': [
                'housing', 'home sales', 'building permits', 'construction',
                'mortgage', 'real estate', 'property'
            ],
            'trade': [
                'trade balance', 'exports', 'imports', 'trade deficit',
                'trade surplus', 'current account', 'trade data'
            ],
            'confidence': [
                'confidence', 'sentiment', 'consumer confidence',
    ...
    def _get_api_key(self) -> str:
        ...
        try:
            with open('/home/rono/THEBOT/api_config.json', 'r') as f:
                ...
                for provider in config.get('providers', {}).get('data_sources', {}).get('economic', []):
    ...
    def get_economic_events(self, 
        ...
        try:
            ...
            logger.info(f"🏛️ Finnhub: Récupération événements {start_date.strftime('%Y-%m-%d')} -> {end_date.strftime('%Y-%m-%d')}")
            ...
            for event in events:
                ...
                if formatted_event and self._passes_filters(formatted_event, countries, impacts):
                    formatted_events.append(formatted_event)
                    
                    if len(formatted_events) >= max_events:
                        break
            
            logger.info(f"✅ Finnhub: {len(formatted_events)} événements formatés après filtrage")
    ...
    def _build_description(self, title: str, estimate: str, actual: str, previous: str) -> str:
    ...
    def _passes_filters(self, event: Dict, countries: List[str], impacts: List[str]) -> bool:
