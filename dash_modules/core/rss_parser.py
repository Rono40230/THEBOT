...
class RSSParser:
    """Parser générique pour flux RSS avec gestion d'erreurs et normalisation"""
    ...
    def _normalize_entry(self, entry: Any, feed_info: Dict[str, str]) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            article = {
                ...
                'provider': 'rss',
                'category': self._determine_category(title, summary, categories, feed_info),
                'tags': categories,
    ...
    def _determine_category(self, title: str, summary: str, tags: List[str], feed_info: Dict[str, str]) -> str:
        """Détermine la catégorie principale de l'article"""
        content = f"{title} {summary}".lower()
        
        # Catégories crypto
        crypto_keywords = ['bitcoin', 'ethereum', 'crypto', 'blockchain', 'defi', 'nft', 'altcoin']
        if any(keyword in content for keyword in crypto_keywords) or any(keyword in tags for keyword in crypto_keywords):
            return 'crypto'
        
        # Catégories économiques
        economic_keywords = ['economy', 'inflation', 'gdp', 'federal reserve', 'interest rate', 'unemployment']
        if any(keyword in content for keyword in economic_keywords):
            return 'economic'
        
        # Catégories marchés
        market_keywords = ['stock', 'market', 'trading', 'investment', 'portfolio', 'earnings']
