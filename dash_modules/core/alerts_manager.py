...
class AlertsManager:
    ...
    def load_alerts(self) -> List[Dict[str, Any]]:
        ...
        try:
            if os.path.exists(self.alerts_file):
                ...
                print(f"✅ {len(self.alerts)} alertes chargées depuis {self.alerts_file}")
            else:
                self.alerts = []
                print(f"📝 Aucun fichier d'alertes trouvé, initialisation avec une liste vide")
    ...
    def save_alerts(self) -> bool:
        ...
        try:
            ...
            print(f"💾 {len(self.alerts)} alertes sauvegardées dans {self.alerts_file}")
            return True
        except Exception as e:
            print(f"❌ Erreur lors de la sauvegarde des alertes: {e}")
            return False
    
    def add_alert(self, symbol: str, alert_type: str, price: float, message: str = "") -> Dict[str, Any]:
        ...
        print(f"✅ Nouvelle alerte ajoutée: {symbol} {alert_type} {price}")
        return alert
    ...
    def get_alerts_by_symbol(self, symbol: str) -> List[Dict[str, Any]]:
        ...
        return [alert for alert in self.alerts if alert['symbol'] == symbol and alert.get('active', True)]
    ...
    def delete_alert(self, alert_id: int) -> bool:
        ...
        try:
            ...
            print(f"❌ Erreur lors de la suppression de l'alerte {alert_id}: {e}")
            return False
        ...
        try:
            for alert in self.alerts:
                if alert['id'] == alert_id:
                    ...
                    self.save_alerts()  # Sauvegarde automatique
                    print(f"✅ Alerte {alert_id} mise à jour")
                    return True
            
            print(f"❌ Alerte {alert_id} non trouvée pour mise à jour")
            return False
        except Exception as e:
    ...
    def clear_all_alerts(self) -> bool:
        ...
        try:
            ...
            print(f"❌ Erreur lors de la suppression de toutes les alertes: {e}")
            return False
