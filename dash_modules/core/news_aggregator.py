...
class NewsAggregator:
    ...
    def get_aggregated_news(self,
                          ...
                          include_apis: bool = True) -> List[Dict[str, Any]]:
        """
        ...
        """
        try:
            logger.info(f"📰 Aggregating news - Categories: {categories}, Limit: {limit}")
            ...
            with ThreadPoolExecutor(max_workers=3) as executor:
                ...
                # Source RSS
                if include_rss:
                    rss_categories = self._map_categories_to_rss(categories) if categories else None
                ...
                if include_apis:
                    future_apis = executor.submit(
                        self._get_api_news, categories, limit
                    )
                ...
                for source_type, future in futures:
                    try:
                        ...
                        if articles:
                            ...
                            logger.debug(f"✅ Got {len(articles)} articles from {source_type}")
    ...
    def _get_rss_news(self, categories: List[str], sources: List[str], limit: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                if yahoo_news:
                    ...
                    logger.debug(f"✅ Got {len(yahoo_news)} articles from Yahoo Finance API")
            except Exception as e:
        ...
        for article in articles:
            try:
                ...
                normalized_article = {
                    ...
                    'timestamp': article.get('timestamp', datetime.now(timezone.utc).isoformat())
                }
                
                # Champs optionnels préservés
                optional_fields = ['tags', 'language', 'feed_title', 'rss_source_name', 'rss_category', 'rss_description']
        ...
        try:
            ...
            stats = {
                'rss_sources': {
                    ...
                    'active_sources': rss_sources_config.get_source_count(active_only=True),
    ...
    def test_all_sources(self) -> Dict[str, Any]:
        ...
        try:
            ...
            results['summary'] = {
                ...
                'overall_health': 'good' if (results['rss_test'].get('successful', 0) > 0) else 'poor'
            }
            
            logger.info(f"✅ Source test completed: RSS {results['summary']['successful_rss']}/{results['summary']['total_rss_sources']}, APIs {results['summary']['successful_apis']}/{results['summary']['total_api_sources']}")
