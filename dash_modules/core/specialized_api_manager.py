...
from ..core.intelligent_cache import cached_api_call, get_global_cache
...
class SpecializedAPIManager:
    ...
    def _detect_market_type(self, symbol: str) -> str:
        ...
        # Crypto patterns
        crypto_patterns = [
            'BTC', 'ETH', 'ADA', 'XRP', 'DOT', 'LINK', 'UNI', 'DOGE', 'SHIB',
            'USDT', 'USDC', 'BUSD', 'DAI', 'MATIC', 'SOL', 'AVAX', 'LUNA',
            'ATOM', 'FTT', 'NEAR', 'ALGO', 'VET', 'ICP', 'THETA', 'FIL'
        ]
    ...
    def get_optimized_data(self, symbol: str, timeframe: str = '1h', limit: int = 100) -> Optional[List[Dict]]:
        """
        ...
        try:
            ...
            if not provider:
                logger.error(f"❌ Provider {provider_name} non disponible pour {symbol}")
                return None
            ...
            if provider_name == 'binance':
                ...
                logger.warning(f"⚠️ Provider {provider_name} non spécialisé pour {symbol}")
                return None
    ...
    def _get_binance_data(self, provider, symbol: str, timeframe: str, limit: int) -> Optional[List[Dict]]:
    ...
    def _get_coingecko_data(self, provider, symbol: str, timeframe: str, limit: int) -> Optional[List[Dict]]:
        ...
        try:
            ...
            symbol_to_id = {
                ...
                'UNI': 'uniswap', 'DOGE': 'dogecoin', 'MATIC': 'polygon'
            }
    ...
    def _get_twelve_data(self, provider, symbol: str, timeframe: str, limit: int) -> Optional[List[Dict]]:
    ...
    def get_optimized_news(self, categories: List[str] = None, limit: int = 20) -> List[Dict]:
        """
        ...
        try:
            ...
            if remaining_limit > 0:
                # Binance pour crypto
                if categories and 'crypto' in categories and 'binance' in self.providers:
                    try:
                        binance_news = self.providers['binance'].get_news(limit=remaining_limit//2)
                        if binance_news:
                ...
                if categories and 'crypto' in categories and 'coin_gecko' in self.providers:
                    try:
                        gecko_updates = self.providers['coin_gecko'].get_market_updates(limit=remaining_limit//2)
                        if gecko_updates:
