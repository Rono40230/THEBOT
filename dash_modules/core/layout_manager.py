        ...
        return html.Footer([
            dbc.Row([
                dbc.Col([
                    html.Small([
                        "THEBOT v2.0 | ",
                        html.Span(id="connection-status", children="🟢 Connected"),
                        " | Last Update: ",
                ...
                dbc.Col([
                    html.Small([
                        ...
                        html.Span(id="data-quality", children="🟢 Good")
...
# create_modular_content supprimé - conflit d'ID résolu
    ...
    def _get_default_module_content(self, modules: Dict = None) -> html.Div:
        ...
        try:
            ...
            return html.Div([
                ...
                html.P("⚠️ Module en cours de chargement...", className="text-warning"),
                html.Div([
    ...
    def _generate_module_placeholder(self, module_name: str) -> html.Div:
        ...
        return html.Div([
            html.H2(f"📊 Module {module_name.title()}", className="text-center mb-4"),
            html.Div([
                html.P(f"✅ Module {module_name} chargé avec succès", className="text-center mb-3"),
                html.Div([
                    dbc.Badge("🔄 Architecture MVC", color="success", className="me-2"),
                    dbc.Badge("⚡ Callbacks actifs", color="info", className="me-2"),
                    dbc.Badge("📊 Données en temps réel", color="warning")
                ], className="text-center")
            ], className="p-4 bg-light rounded")
        ], className="container-fluid p-4")
    
    def get_main_layout(self, all_symbols: List[str], default_data: Dict[str, Any], modules: Dict = None) -> dbc.Container:
        """
        ...
        try:
            ...
            return dbc.Container([
                ...
                dbc.Row([
                    ...
                    ], width=0, className="pe-0", id="sidebar-col"),  # Largeur 0 par défaut
                    
                    # Zone principale modulaire
                    dbc.Col([
                        html.Div(id="modular-content", children=default_content)
                    ], width=12, className="ps-2", id="main-content-col")  # Pleine largeur par défaut
            ...
            return dbc.Container([
                html.Div("Layout Error - Check logs", className="text-danger text-center p-5")
            ])
    ...
    def _create_data_stores(self, all_symbols: List[str], default_data: Dict[str, Any]) -> html.Div:
        """Crée les stores et intervals de données"""
        return html.Div([
            # Stores pour données
            dcc.Store(id='market-data-store', data=default_data.get('market_data', {})),
