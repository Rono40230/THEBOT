...
class SmartAIEngine:
    ...
    def _should_use_paid_ai(self, context: Dict) -> tuple[bool, str]:
        ...
        # Événement majeur
        if context.get('major_news_event', False) and self.paid_ai_triggers['major_news_event']:
        ...
        if context.get('conflicting_signals', False) and self.paid_ai_triggers['conflicting_signals']:
    ...
    def _log_ai_usage(self, service: str, cost: float, context: str):
        ...
        logger.info(f"💰 IA usage: {service} - {cost}€ - Total jour: {self.current_daily_spend:.3f}€")
    
    def analyze_market_comprehensive(self, symbol: str, market_data: Dict, news_data: List[Dict], context: Dict = None) -> Dict:
        ...
        local_insight = local_ai_engine.generate_trading_insight(symbol, {'technical_analysis': local_technical}, local_sentiment)
        ...
        if use_paid_ai and (self.openai_available or self.claude_available):
            try:
                ...
                if enhanced_analysis:
                    ...
                    analysis_result['combined_insight'] = self._combine_analyses(local_insight, enhanced_analysis)
                
            except Exception as e:
                logger.error(f"Erreur IA payante, fallback local: {e}")
                analysis_result['enhanced_analysis'] = {'error': str(e), 'fallback': 'local_only'}
    ...
    def _enrich_context(self, symbol: str, market_data: Dict, news_data: List[Dict], base_context: Dict) -> Dict:
        ...
        if current > 0:
            enriched['volatility_24h'] = round(((high_24h - low_24h) / current) * 100, 2)
        ...
        enriched['conflicting_signals'] = abs(local_sentiment_score - technical_score) > 30
        
        enriched['symbol'] = symbol
        enriched['analysis_complexity'] = 'high' if enriched.get('major_news_event') or enriched.get('volatility_24h', 0) > 3 else 'normal'
    ...
    def _call_paid_ai(self, context: Dict, local_insight: Dict) -> Optional[Dict]:
        ...
        return {
            ...
            'additional_signals': ['macro_correlation', 'institutional_flow', 'psychological_levels'],
    ...
    def _combine_analyses(self, local: Dict, enhanced: Dict) -> Dict:
        ...
        enhanced_confidence = min(95, original_confidence + enhanced.get('confidence_boost', 0))
        ...
        return {
            ...
            'combined_insights': local.get('explanation', '') + " | " + enhanced.get('enhanced_insights', [''])[0],
    ...
    def _get_budget_status(self) -> Dict:
        ...
        return {
            ...
            'daily_remaining': round(self.daily_budget_euro - self.current_daily_spend, 3),
            'monthly_spend': round(self.current_monthly_spend, 2),
            'monthly_budget': self.monthly_budget_euro,
            'monthly_remaining': round(self.monthly_budget_euro - self.current_monthly_spend, 2),
            'usage_level': 'conservative' if self.current_daily_spend < self.daily_budget_euro * 0.5 else 'active'
        }
